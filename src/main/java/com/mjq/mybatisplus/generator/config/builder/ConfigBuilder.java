 ...
 * the License.
 */
package com.mjq.mybatisplus.generator.config.builder;

import com.mjq.mybatisplus.generator.InjectionConfig;
import com.mjq.mybatisplus.generator.annotation.DbType;
import com.mjq.mybatisplus.generator.config.*;
import com.mjq.mybatisplus.generator.config.po.TableField;
import com.mjq.mybatisplus.generator.config.po.TableFill;
import com.mjq.mybatisplus.generator.config.po.TableInfo;
import com.mjq.mybatisplus.generator.config.rules.NamingStrategy;
import com.mjq.mybatisplus.generator.util.StringPool;
...
//            setPathInfo(pathInfo, template.getController(), outputDir, ConstVal.CONTROLLER_PATH, ConstVal.CONTROLLER);
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, StrategyConfig config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            String s = tableInfo.getEntityName();
            tableInfo.setObjectName(new StringBuilder().append(Character.toLowerCase(s.charAt(0))).append(s.substring(1)).toString());
            ...
            if (StringUtils.isNotEmpty(globalConfig.getProviderName())) {
                ...
                tableInfo.setProviderName(entityName + ConstVal.PROVIDER);
            }
    ...
    private TableInfo convertTableFields(TableInfo tableInfo, NamingStrategy strategy) {
        ...
        try {
            ...
            while (results.next()) {
                ...
                //排除不必要非主键字段
                HashSet<String> exclusiveField = strategyConfig.getExcludeField();
                if (exclusiveField.contains(field.getName()) && !field.isKeyFlag()) {
                    continue;
                }

                field.setType(results.getString(dbQuery.fieldType()));
                field.setPropertyName(strategyConfig, processName(field.getName(), strategy));
                field.setCapitalFirstPropertyName(NamingStrategy.capitalFirst(field.getPropertyName()));
                field.setColumnType(dataSourceConfig.getTypeConvert().processTypeConvert(globalConfig, field.getType()));
                field.setComment(results.getString(dbQuery.fieldComment()));
                if (field.isKeyFlag()) {
                    tableInfo.setKeyFieldName(field.getPropertyName());
                    tableInfo.setKeyFieldType(field.getColumnType().getType());
                    tableInfo.setPrimaryKeyName(field.getName());
                    tableInfo.setKeyCapitalFirstFieldName(field.getCapitalFirstPropertyName());
                }
